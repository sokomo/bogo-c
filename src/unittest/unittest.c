...
void assertNotInt (int expected,
                   int expr) {
    /* Skip if there is a failed test already */
    if (_currentRes_ == 0) {
        return;
    ...
    _currentRes_ = (expected != expr) ? 1 : 0;
    if (_currentRes_ == 0) {
        fprintf (stderr, "[failed] assertNotInt\n\texpected: %i\n\tactual: %i\n",
                 expected,
                 expr);
    }
}
...
void assertNotStr (const char *expected,
                   const char *expr) {
    /* Skip if there is a failed test already */
    if (_currentRes_ == 0) {
        return;
    ...
    _currentRes_ = (strcmp (expected, expr) != 0) ? 1 : 0;
    if (_currentRes_ == 0) {
        fprintf (stderr, "[failed] assertNotStr\n\texpected: %s\n\tactual: %s\n",
                 expected, expr);
    }
}
...
void assertNotStrI (const char *expected,
                    const char *expr) {
    /* Skip if there is a failed test already */
    if (_currentRes_ == 0) {
        return;
    ...
    _currentRes_ = (strcmp (lowerExpected, lowerExpr) != 0) ? 1 : 0;
    if (_currentRes_ == 0) {
        fprintf (stderr, "[failed] assertNotStrI\n\texpected: %s\n\tactual: %s\n",
                 expected, expr);
    }
}

void assertStrIDescription (const char *expected,
                            const char *expr,
                            const char *descr) {
    /* Skip if there is a failed test already */
    if (_currentRes_ == 0) {
